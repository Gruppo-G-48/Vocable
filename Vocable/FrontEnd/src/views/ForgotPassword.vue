        ...
        async sendResetLink() {
            ...
            if (valid) {
                try {
                    ...
                    const response = await axios.post('https://vocable-production.up.railway.app/api/utente/forgot-password', { email: this.email });

                    if (response.data.status) {
                        const resetToken = response.data.resetToken;
                       

                        const resetLink = `https://vocable-production.up.railway.app/reset-password/${resetToken}`;
